...
try:
    ...
    if lung_cancer_model is not None:
        print("🔍 Testing lung cancer model feature requirements...")
        model_feature_count = None
        
        for feature_count in range(8, 13):
            try:
                test_features = np.zeros((1, feature_count))
                ...
                print(f"✅ Model accepts {feature_count} features")
                model_feature_count = feature_count
                break
                
            except Exception as e:
                print(f"❌ {feature_count} features failed: {str(e)}")
...
def get_lung_cancer_features(data):
    ...
    feature_combinations = [
        # Option 1: 10 features without sex (most likely)
        {
            'features': [
                ...
                float(data.get('ros1', 0)),
                float(data.get('met', 0))
            ],
            'names': LUNG_CANCER_FEATURES_10,
            'description': '10 features: Age + 9 genetic mutations'
        },
        # Option 2: 10 features with sex, without MET
        {
            'features': [
                float(data.get('age', 0)),
            ...
            'names': LUNG_CANCER_FEATURES_ALT1,
            'description': '10 features: Age + Sex + 8 genetic mutations (no MET)'
        },
        # Option 3: 10 features with sex, without ROS1
        {
            'features': [
                float(data.get('age', 0)),
                float(data.get('sex', 0)),
...
chatbot_retriever = None

def initialize_chatbot():
    """Initialize the chatbot retriever"""
    global chatbot_retriever
    ...
    try:
        ...
        if not os.path.exists(context_dir):
            print(f"⚠️ Knowledge base directory '{context_dir}' not found. Creating basic responses.")
            return False
        ...
        if not txt_files:
            print(f"⚠️ No .txt files found in '{context_dir}'. Using basic responses.")
            return False
        ...
        if not all_documents:
            print("⚠️ No documents were loaded successfully. Using basic responses.")
            return False

        index = VectorStoreIndex.from_documents(all_documents)
        chatbot_retriever = index.as_retriever(similarity_top_k=3, verbose=False)
        
        print(f"✅ Chatbot initialized successfully with {len(all_documents)} documents!")
        return True
...
def get_db_connection():
    """Create database connection with multiple fallback methods"""
    try:
        print("🔗 Attempting database connection...")
        
        # Method 1: Try with URL encoding for special characters
        server_name = urllib.parse.quote_plus(DB_CONFIG['server'])
        driver_name = urllib.parse.quote_plus(DB_CONFIG['driver'])
        
        connection_string = f"mssql+pyodbc://@{server_name}/{DB_CONFIG['database']}?driver={driver_name}&trusted_connection=yes"
        ...
        # Method 2: Try alternative connection string format
        try:
            ...
            print(f"🔗 Attempting alternative connection: {connection_string}")
            ...
            try:
                connection_string = "mssql+pyodbc://localhost\\SQLEXPRESS/PrecisionMedicine?driver=ODBC+Driver+17+for+SQL+Server&trusted_connection=yes"
                print(f"🔗 Attempting localhost connection: {connection_string}")
                ...
                # Method 4: Try with different drivers
                try:
                    # Try with SQL Server driver
                    connection_string = "mssql+pyodbc://YESWANTH\\SQLEXPRESS/PrecisionMedicine?driver=SQL+Server&trusted_connection=yes"
                    print(f"🔗 Attempting SQL Server driver: {connection_string}")
                    
                    engine = create_engine(connection_string, echo=False)
                    
                    with engine.connect() as conn:
                        conn.execute(text("SELECT 1"))
                        print("✅ SQL Server driver connection successful!")
                        return engine
...
def save_lung_cancer_inputs(conn, prediction_id, input_data):
    ...
    try:
        ...
        query = text("""
            INSERT INTO LungCancerInputs 
            (PredictionID, Gender, Age, EGFRMutation, KRASMutation, ALKMutation, TP53Mutation, STK11Mutation, KEAP1Mutation, BRAFMutation, ROS1Mutation, METMutation)
            VALUES (:prediction_id, :gender, :age, :egfr_mutation, :kras_mutation, :alk_mutation, :tp53_mutation, :stk11_mutation, :keap1_mutation, :braf_mutation, :ros1_mutation, :met_mutation)
        """)
...
def get_lung_cancer_recommendations(health_score, prediction_proba, features, input_data):
    ...
    feature_names = ['age', 'egfr', 'kras', 'alk', 'tp53', 'stk11', 'keap1', 'braf', 'ros1', 'met']
...
def chat_endpoint():
    ...
    try:
        data = request.json
        # Support both 'message' and 'question' fields for compatibility
        ...
        if chatbot_retriever is None:
            initialize_chatbot()
        
        # Get chatbot response
        if chatbot_retriever is not None:
            # Retrieve relevant nodes
            try:
                retrieved_nodes = chatbot_retriever.retrieve(message)
            except Exception as e:
                print(f"⚠️ Retrieval error: {e}")
                retrieved_nodes = []
            
            # Get response type and content
            response_type, greeting_key = smart_bot.get_response_type(message, retrieved_nodes)
            
            if response_type == 'greeting':
                response = smart_bot.greetings[greeting_key]
            elif response_type == 'thanks':
                response = random.choice(smart_bot.thanks_responses)
            elif response_type == 'goodbye':
                response = smart_bot.goodbye_response
            elif response_type == 'knowledge' and retrieved_nodes:
                # Use the best retrieved answer
                best_node = retrieved_nodes[0]
                if hasattr(best_node, 'metadata') and 'answer' in best_node.metadata:
                    response = best_node.metadata['answer']
                else:
                    response = best_node.text
            else:
                response = random.choice(smart_bot.unknown_responses)
        else:
            # Fallback responses when retriever is not available
            greeting = smart_bot.is_greeting(message)
            if greeting:
                response = smart_bot.greetings[greeting]
            elif smart_bot.is_thanks(message):
                response = random.choice(smart_bot.thanks_responses)
            elif smart_bot.is_goodbye(message):
                response = smart_bot.goodbye_response
            else:
                response = random.choice(smart_bot.unknown_responses)
